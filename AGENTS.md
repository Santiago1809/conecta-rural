...
Points economy: `COMMENT_POINTS = 25` and `LIKE_POINTS = 15` (`lib/db.ts`) are hardcoded and the README restates them — change both. Balance is a ledger (`point_transactions`), never a mutable column. `point_transactions.comment_id` is `UNIQUE` and already claimed by the publish transaction, so the like ledger rows leave it `NULL` and attribution lives in `comment_likes`. The discount catalog is seeded with `INSERT OR IGNORE` from the first 3 `ALOJAMIENTOS`, so **editing the seed numbers never updates an existing row** — `migrateDiscountCosts()` in `ensureDatabase()` is what reprices a deployed database, and it is idempotent because each statement sets a constant and matches every cost we have ever shipped (`DISCOUNT_COSTS` in `lib/db.ts` lists the superseded values per entry), so a row converges in one call and a later call matches nothing. Never rewrite it as `points_cost = points_cost * 1.5`: `ensureDatabase()` runs on nearly every request, so that would compound forever. Note each rebalance applies to the previous rebalance's **output**, not the original base, so 113 → 226 is correct and 75 × 3 → 225 is not. `redemptions` has `UNIQUE(user_id, discount_id)`, so a discount is redeemable once per user by design; redeemed discounts are historical records and are never repriced or refunded.
